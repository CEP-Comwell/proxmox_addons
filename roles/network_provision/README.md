...
### Persistent Interface Naming: .link vs .rules (Proxmox VE 8/9)

#### .link Files (Preferred, Proxmox VE 8+)
Location: `/etc/systemd/network/`
```
10-pve-<interface>.link
```
These are generated by `pve-network-interface-pinning` and are the preferred method for persistent interface naming. If this tool succeeds, no udev rules are needed.

#### .rules Files (Fallback, Proxmox VE 9)
Location: `/etc/udev/rules.d/`
```
70-pve-net-<interface>.rules
```
If `pve-network-interface-pinning` fails (as is common in Proxmox VE 9), the role automatically generates udev rules for persistent naming. This fallback is fully supported and expected.

#### How the Role Decides Which to Use
- The role first attempts to use `pve-network-interface-pinning` for each interface.
- If it fails (non-zero return code), it generates a udev rule for that interface.
- All failures are tracked in the `pinning_failures` variable for reporting.
- Udev rules are applied immediately with `udevadm control --reload-rules && udevadm trigger`.

#### Rollback Instructions
To revert persistent naming changes:
- **For .link files:**
  1. Remove the relevant `/etc/systemd/network/10-pve-<interface>.link` file(s).
  2. Reboot the node.
- **For .rules files:**
  1. Remove the relevant `/etc/udev/rules.d/70-pve-net-<interface>.rules` file(s).
  2. Run `udevadm control --reload-rules && udevadm trigger`.
  3. Reboot the node if interface names do not revert immediately.

**Note:** Removing both .link and .rules files and rebooting will restore default kernel interface names.
